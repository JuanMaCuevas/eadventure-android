...
public class Game implements TimerEventListener, SpecialAssetPaths {
	 ...
	 * Communication api
	 */
	// OLD private AdventureApplet comm;

	/**
	 * Last mouse event of the game
	 */
	// OLD private MouseEvent lastMouseEvent;
	 ...
	 */
	// OLD private MouseEvent lastPressedEvent;
	...
	public void start() {
		...
		try {
			...
			if (needsName) {
				...
				// String name = JOptionPane.showInputDialog( null, TC.get(
				// "Reports.InputReportName" ), TC.get( "Reports.NameInput" ),
			...
			FileWriter file = new FileWriter(Paths.eaddirectory.ROOT_PATH
					+ "memoryLog.csv");

			while (!gameOver) {
				// TODO esto es solo para probar que caga bien el juego
				// FIXME habra que cambiar varias cosas
				// 2) que no haga un loadcurrentchapter, si lo pongo despues me
				// peta xq algo no esta inicializado
				...
				while (!nextChapter && !gameOver) {
					...
					if (this.globalState == STATE_RUNNING) {
						...
						// generation of log data about the usage of native heap
						// allocation
						// csv file with the fields: free memory, allocated
						// memory, heap size
	...
	private void writeMemoryUsageLog(FileWriter file, long elapsedTime) {
		...
		if (totalElapsedTime > refresh) {
			...
			try {
				...
				String memorylog = currentState.toString() + ","
						+ Long.toString(Debug.getNativeHeapFreeSize()) + ","
	...
	private void loadCurrentChapter() {
		...
		MultimediaManager.getInstance().flushImagePool(
				MultimediaManager.IMAGE_SCENE);
		MultimediaManager.getInstance().flushImagePool(
				MultimediaManager.IMAGE_PLAYER);
		...
		/*
		 * ADAPTATION AdaptedState initialState=null; // Load the assessment
		 * rules and adaptation data if (gameData.hasAdaptationProfile())
		 * initialState = adaptationEngine.init(
		 * gameData.getSelectedAdaptationProfile() );
		 */
		if (gameData.hasAssessmentProfile())
			assessmentEngine.loadAssessmentRules(gameData
					.getSelectedAssessmentProfile());

		/*
		 * ADAPTATION // Load the assessment rules and adaptation data (from
		 * chapter xml file) if
		 * (!gameData.hasAdaptationProfile()&&chapter.hasAdaptationProfile())
		 * initialState = adaptationEngine.init(
		 * chapter.getSelectedAdaptationProfile() );
		 */

		if (!gameData.hasAssessmentProfile() && chapter.hasAssessmentProfile())
			assessmentEngine.loadAssessmentRules(chapter
					.getSelectedAssessmentProfile());
		...
		// Initialize the stack that store if each list of effects keep in
		// effectsQueue is made in a conversation or not
		isConvEffectsBlock = new Stack<Boolean>();

		numberConv = 0;

		// Initialize the stack of states (used to keep the conversations and
		// can throw its effects)
		...
		/*
		 * ADAPTATION // process the initial adapted state processAdaptedState(
		 * firstScene, initialState ); // process the adaptedStateOfExecute
		 * (this var will has value if any adaptation rule has been achieve)
		...
		functionalPlayer
				.setTransparent(gameDescriptor.getPlayerMode() == DescriptorData.MODE_PLAYER_1STPERSON);
		...
		// to know if there are gps events
		// activas el gps
		if (gameData.doesGpsExist()) {
	...
	private void preLoadAnimations() {
		...
		for (Resources r : gameData.getPlayer().getResources()) {
			multimedia.loadAnimation(r
					.getAssetPath(NPC.RESOURCE_TYPE_STAND_RIGHT), false,
					MultimediaManager.IMAGE_PLAYER);
			if (r.getAssetPath(NPC.RESOURCE_TYPE_STAND_LEFT) != null
					&& !r.getAssetPath(NPC.RESOURCE_TYPE_STAND_LEFT).equals(
							ASSET_EMPTY_ANIMATION))
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_STAND_LEFT), false,
						MultimediaManager.IMAGE_PLAYER);
			else
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_STAND_RIGHT), true,
						MultimediaManager.IMAGE_PLAYER);
			multimedia.loadAnimation(
					r.getAssetPath(NPC.RESOURCE_TYPE_STAND_UP), false,
					MultimediaManager.IMAGE_PLAYER);
			multimedia.loadAnimation(r
					.getAssetPath(NPC.RESOURCE_TYPE_STAND_DOWN), false,
					MultimediaManager.IMAGE_PLAYER);

			multimedia.loadAnimation(r
					.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_RIGHT), false,
					MultimediaManager.IMAGE_PLAYER);
			if (r.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_LEFT) != null
					&& !r.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_LEFT).equals(
							ASSET_EMPTY_ANIMATION))
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_LEFT), false,MultimediaManager.IMAGE_PLAYER);
			else
				multimedia.loadAnimation(r.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_RIGHT), true,
						MultimediaManager.IMAGE_PLAYER);
			multimedia.loadAnimation(
					r.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_UP), false,
					MultimediaManager.IMAGE_PLAYER);
			multimedia.loadAnimation(r
					.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_DOWN), false,
					MultimediaManager.IMAGE_PLAYER);

			multimedia.loadAnimation(r
					.getAssetPath(NPC.RESOURCE_TYPE_USE_RIGHT), false,
					MultimediaManager.IMAGE_PLAYER);
			if (r.getAssetPath(NPC.RESOURCE_TYPE_USE_LEFT) != null
					&& !r.getAssetPath(NPC.RESOURCE_TYPE_USE_LEFT).equals(
							ASSET_EMPTY_ANIMATION))
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_USE_LEFT), false,
						MultimediaManager.IMAGE_PLAYER);
			else
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_USE_RIGHT), true,
						MultimediaManager.IMAGE_PLAYER);

			multimedia.loadAnimation(r
					.getAssetPath(NPC.RESOURCE_TYPE_WALK_RIGHT), false,
					MultimediaManager.IMAGE_PLAYER);
			if (r.getAssetPath(NPC.RESOURCE_TYPE_WALK_LEFT) != null
					&& !r.getAssetPath(NPC.RESOURCE_TYPE_WALK_LEFT).equals(
							ASSET_EMPTY_ANIMATION))
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_WALK_LEFT), false,
						MultimediaManager.IMAGE_PLAYER);
			else
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_WALK_RIGHT), true,
						MultimediaManager.IMAGE_PLAYER);
			multimedia.loadAnimation(r.getAssetPath(NPC.RESOURCE_TYPE_WALK_UP),
					false, MultimediaManager.IMAGE_PLAYER);
			multimedia.loadAnimation(r
					.getAssetPath(NPC.RESOURCE_TYPE_WALK_DOWN), false,
					MultimediaManager.IMAGE_PLAYER);
		}
		for (NPC npc : gameData.getCharacters()) {
			for (Resources r : npc.getResources()) {
				multimedia.loadAnimation(r
						...
						MultimediaManager.IMAGE_SCENE);
				if (r.getAssetPath(NPC.RESOURCE_TYPE_STAND_LEFT) != null
						&& !r.getAssetPath(NPC.RESOURCE_TYPE_STAND_LEFT)
								.equals(ASSET_EMPTY_ANIMATION))
					multimedia.loadAnimation(r
							.getAssetPath(NPC.RESOURCE_TYPE_STAND_LEFT), false,
							MultimediaManager.IMAGE_SCENE);
				else
					multimedia.loadAnimation(r
							...
							MultimediaManager.IMAGE_SCENE);
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_STAND_UP), false,
						MultimediaManager.IMAGE_SCENE);
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_STAND_DOWN), false,
						MultimediaManager.IMAGE_SCENE);
				...
				multimedia.loadAnimation(r
						...
						MultimediaManager.IMAGE_SCENE);
				if (r.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_LEFT) != null
						&& !r.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_LEFT)
								.equals(ASSET_EMPTY_ANIMATION))
					multimedia.loadAnimation(r
							.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_LEFT), false,
							MultimediaManager.IMAGE_SCENE);
				else
					multimedia.loadAnimation(r
							.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_RIGHT), true,
							MultimediaManager.IMAGE_SCENE);
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_UP), false,
						MultimediaManager.IMAGE_SCENE);
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_SPEAK_DOWN), false,
						MultimediaManager.IMAGE_SCENE);
				...
				if (r.getAssetPath(NPC.RESOURCE_TYPE_WALK_LEFT) != null
					...
					multimedia.loadAnimation(r
							...
							MultimediaManager.IMAGE_SCENE);
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_WALK_UP), false,
						MultimediaManager.IMAGE_SCENE);
				multimedia.loadAnimation(r
						.getAssetPath(NPC.RESOURCE_TYPE_WALK_DOWN), false,
						MultimediaManager.IMAGE_SCENE);
	...
	private void processAdaptedState(Exit firstScene, AdaptedState adaptedState) {
		...
		if (adaptedState != null) {
				...
				if (!found) {
					...
					for (Cutscene cutscene : gameData.getCutscenes()) {
						if (cutscene.getId().equals(adaptedState.getTargetId()))
							firstScene.setNextSceneId(adaptedState
									.getTargetId());
			...
			for (int i = 0; i < adaptedVars.size(); i++) {
				...
				// it is "increment" or "decrement" operation, for both of them
				// is necessary to
				// get the current value of referenced variable
				else {
					if (vars.existVar(varName)) {
						int currentValue = vars.getValue(varName);
						int operationValue = Integer.parseInt(varValue
								.substring(varValue.indexOf(" ") + 1));
						if (AdaptedState.isIncrementOp(varValue.substring(0,
								varValue.indexOf(" ")))) {
							vars.setVarValue(varName, currentValue
									+ operationValue);
						} else if (AdaptedState.isDecrementOp(varValue
								.substring(0, varValue.indexOf(" ")))) {
	 ...
	 */
	public synchronized void updateDataPendingFromState(
			boolean notifyTimerCycles) {
	...
	public void grabItem(String itemId) {
		...
		// Remove the FunctionalItem from the scene and store it into the
		// inventory
	...
	public void pushCurrentState(GameState gs) {
		...
		// store the name of the conversation for future conversation restoring.
	...
	public void save(String saveFile) {
		...
		saveGame.setVersionNumber(Integer.parseInt(gameDescriptor
				.getVersionNumber()));
		...
		saveGame.setSaveTime(calendar.get(Calendar.DAY_OF_MONTH) + "/"
		...
		if (!saveGame.saveTxt(saveFile))
			System.out
					.println("* Error: There has been an error, savegame ''savedgame.egame'' not saved.");
	}
		...
		try {
			if (saveGame.loadTxt(saveFile)) {
				...
				if (gameDescriptor.getTitle().equals(saveGame.getTitle())) {
					...
					if (gameDescriptor.getChapterSummaries()
							.get(currentChapter) != null) {
						ChapterSummary chapter = gameDescriptor
								.getChapterSummaries().get(currentChapter);
						gameData = Loader.loadChapterData(ResourceHandler
								.getInstance(), chapter.getChapterPath(),
								new ArrayList<Incidence>(), true);
					}
					...
					if (gameData.getGeneralScene(saveGame.getIdScene()) != null)
						functionalScene = new FunctionalScene((Scene) gameData
								.getGeneralScene(saveGame.getIdScene()),
								functionalPlayer);
					...
					inventory = new Inventory();
					ArrayList<String> grabbedItems = itemSummary
							.getGrabbedItems();

					for (String item : grabbedItems)
						inventory.storeItem(new FunctionalItem(gameData
								.getItem(item), (InfluenceArea) null));
					...
					if (timers != null) {
						for (int i = 0; i < timers.length; i++) {
							...
							boolean isAssessment = Integer.valueOf(aux[3])
									.intValue() == 0;
							st.setState(Integer.valueOf(aux[0]).intValue());
							if (timerManager.isRunningState(Integer.valueOf(
									aux[0]).intValue())) {
								st
										.setLastUpdate(System
												.currentTimeMillis() / 1000);
								if (!isAssessment)
									st.setTimeUpdate(Integer.valueOf(aux[1])
											.longValue()
							...
							if (check >= 0) {
								...
								System.out
										.println("* Error: There has been an error, savegame ''savedgame.egame'' not propperly loaded.");
							}
							// If it is assessment timer, set the correct values
							// in assessmentEngine
							if (isAssessment) {
								// current time - the time in second that has
								// been
								if (assessmentEngine
										.getTimedAssessmentRule(new Integer(i)) != null)
									assessmentEngine.getTimedAssessmentRule(
											new Integer(i)).setStartTime(
					...
					System.out
							.println("* Error: There has been an error, savegame ''savedgame.egame'' not loaded.");
				}
			}
		} catch (SaveGameException e) {
			Log.i("Game.load",
					"There was an error while loading the selected adventure");
			JOptionPane
					.showMessageDialog(
							null,
							"There was an error while loading the selected adventure.\nPlease check that no configuration file is missing or incorrect",
							"Error loading adventure",
							JOptionPane.ERROR_MESSAGE);
			JOptionPane.showMessageDialog(null,
					"The first chapter will be reloaded",
					"Error loading adventure", JOptionPane.ERROR_MESSAGE);
	...
	public FunctionalEffect getFirstElementOfTop() {
		...
		// Con esto que esta comentado no solo avanzo en la cola, sino en la
		// pila tb
		/*
		 * if (effectsQueue.peek().size()==1 && effectsQueue.size()!=1){
		 * FunctionalEffect fe = effectsQueue.peek().remove(0);
		 * popEffectsStack(); return fe; }else return
		 * effectsQueue.peek().remove(0);
		 */
	 ...
	 * 
	 * if( comm != null ) { return comm.isConnected( ); } return false; }
	 */
	...
	public void setState(int state) {
		...
		case STATE_RUN_EFFECTS:
			currentState = new GameStateRunEffects(this.isConvEffectsBlock
					.peek());
			break;
	 ...
	 */
	public void setCharacterCurrentlyTalking(
			TalkingElement characterCurrentlyTalking) {
