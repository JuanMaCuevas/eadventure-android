...
//                temp = image.getScaledInstance( Math.round( image.getWidth( null ) * scale ), Math.round( image.getHeight( null ) * scale ), Image.SCALE_SMOOTH );
    ...
    public boolean dragTo( FunctionalItem anotherItem ) {
        ...
        for( int i = 0; i < item.getActions( ).size( ) && !dragTo; i++ ) {
            Action action = item.getAction( i );
            if( action.getType( ) == Action.DRAG_TO ) {
            	if ( action.getTargetId( ).equals( anotherItem.getItem( ).getId( ) ) ) {
            		if( new FunctionalConditions( action.getConditions( ) ).allConditionsOk( ) ) {
            			// Store the effects
            			FunctionalEffects.storeAllEffects( action.getEffects( ) );
            			dragTo = true;
                ...
                //OLD INICIO
                // the target is not the same as defined at authoring, check if the conditions are meet
                // to skip looking for the first action with not effects and execute the fail click effects
            	} else {
                       if( action.isActivatedFailClickEffects() && new FunctionalConditions( action.getConditions( ) ).allConditionsOk( ) ) 
                           return false;
                }
            }
            	//OLD FIN
    ...
    public boolean dragTo( FunctionalNPC npc ) {
        ...
        for( int i = 0; i < item.getActions( ).size( ) && !dragTo; i++ ) {
            Action action = item.getAction( i );
            if( action.getType( ) == Action.DRAG_TO ){ 
            	if (action.getTargetId( ).equals( npc.getNPC( ).getId( ) ) ) {
            		if( new FunctionalConditions( action.getConditions( ) ).allConditionsOk( ) ) {
            			// Store the effects
            			FunctionalEffects.storeAllEffects( action.getEffects( ) );
            			dragTo = true;
