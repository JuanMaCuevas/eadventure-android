...
public class FunctionalPlayer extends FunctionalElement implements TalkingElement {
    ...
    //OLD INICIO
    /**
     * Given the functional element, trigger the click fail effects assp
     * @param element
     */
    public void triggerClickFailEffects(FunctionalElement element){
        
        int actionType = Game.getInstance( ).getActionManager( ).getActionSelected( );
        switch( actionType ) {
     
            case ActionManager.ACTION_DRAG_TO:
                new FunctionalDragTo( element ).launchClickFailEffects( );
                break;    
