...
public abstract class GUI {
	...
	/**
	 * The default cursor No cursor in the android port
	 */
	...
	protected class SceneImage {
		...
		public void draw(Canvas c) {
			...
			c.drawBitmap(background, new Rect(offsetX, 0, offsetX
					+ GUI.WINDOW_WIDTH, GUI.WINDOW_HEIGHT), new Rect(0, 0,
					GUI.WINDOW_WIDTH, GUI.WINDOW_HEIGHT), null);
		}
	}
		...
		public void draw(Canvas c) {
			...
			if (showBubble)
				GUI.drawStringOnto(c, text, x, y, textColor, borderColor,
						bubbleBkgColor, bubbleBorderColor, showArrow);
			else
				GUI.drawStringOnto(c, text, x, y, textColor, borderColor);
		}
	...
	public static void create() {
		...
		instance = new GUIFrame();
		  ...
		  if( GUIType == GUI.GUI_APPLET ) instance = new GUIApplet( ); else
	...
	// OLD public abstract JFrame showComponent( Component component, int w, int
	// h );
	...
	public int getGameAreaWidth() {
		...
		// TODO hud return hud.getGameAreaWidth( );
	}
	...
	public int getGameAreaHeight() {
		...
		// TODO Hud return hud.getGameAreaHeight( );
	}
	...
	public void update(long elapsedTime) {
		...
		// TODO HUD hud.update( elapsedTime );
	}
	 ...
	 */
	public static void drawStringOnto(Graphics2D g, String string, int x,
			int y, boolean centeredX, Color textColor, Color borderColor,
			boolean border) {
	 ...
	 *            Color of the border of the text
	 */
	public static void drawStringOnto(Graphics2D g, String[] strings, int x,
			int y, Color textColor, Color borderColor) {

		// Calculate the total height of the block text
		FontMetrics fontMetrics = g.getFontMetrics();
		int textBlockHeight = fontMetrics.getHeight() * strings.length
	 ...
	 */
	public static void drawStringOnto(Graphics2D g, String[] strings, int x,
			int y, Color textColor, Color borderColor, Color bkgColor,
			Color bubbleBorder, boolean showArrow) {
		...
		int maxWidth = 25;
		for (String line : strings)
			maxWidth = (fontMetrics.stringWidth(line) > maxWidth ? (int) fontMetrics
					.stringWidth(line)
					: maxWidth);
	 ...
	 */
	public static void drawString(Canvas c, String string, int x, int y) {
		...
		// Draw the string
		// OLD g.drawString( string, realX, realY );

		float cx = x;
		float cy = y;
		c.drawText(text, cx, cy, new Paint());
	...
	public void drawScene(Canvas g, long elapsedTime) {
		...
		if (transition != null && !transition.hasStarted()) {
			...
			gameFrame.paintAll(graph); // graph.dispose();
	...
	public void drawToGraphics(Canvas g) {
		...
		if (showsOffsetArrows) {
			...
			g.drawLine(5, ytemp, 15, ytemp + 15);
			g.drawLine(GUI.WINDOW_WIDTH - 5, ytemp, GUI.WINDOW_WIDTH - 15,
					ytemp - 15);
			g.drawLine(GUI.WINDOW_WIDTH - 5, ytemp, GUI.WINDOW_WIDTH - 15,
					ytemp + 15);
	...
	public void setCursor() {// Cursor cursor ) {
		...
		// OLD CURSOR gameFrame.setCursor( cursor );
	}
	...
	public void setDefaultCursor() {
		...
		// OLD gameFrame.setCursor( defaultCursor );
	}
	 ...
	 *            Offset of the background
	 */
	public void addForegroundToDraw(Bitmap foreground, int offsetX) {
	...
	public void prepareVirtualElemetsToDraw() {
		virtualElementsToDraw = new ArrayList<ElementImage>(); // iterate
																// elemetsToDraw
		for (int i = 0; i < elementsToDraw.size(); i++)
			if (virtualElementsToDraw.size() == 0)
				virtualElementsToDraw.add(elementsToDraw.get(i));
			else {
				boolean added = false; // iterate virtualElementsToDraw, to
										// inserts the next element to iterate
										// in elementsToDraw in the correct
										// position
				int j = 0;
				while (!added && j < virtualElementsToDraw.size()) {
					if (elementsToDraw.get(i).getOriginalY() <= virtualElementsToDraw
							.get(j).getOriginalY()) {
	...
	public static void setGraphicConfig(int newGraphicConfig) {
		...
		// OLD graphicConfig = newGraphicConfig;
	}

	// OLD public abstract Frame getJFrame( );
	...
	public void setTransition(int transitionTime, int transitionType,
		...
		if (transitionTime > 0 && transitionType > 0) {
			// TODO this.transition = new Transition( transitionTime,
			// transitionType );
		}
	...
	public void setLastMouseMove() {// TODO MouseEvent e ) {
		...
		if (percent == 0) {
			...
			if (this.loadingImage == null) {
				this.loadingImage = MultimediaManager.getInstance()
						.loadImageFromZip("gui/loading.jpg",
