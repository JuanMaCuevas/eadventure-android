...
public class DraggingState extends HUDstate {
    ...
    private boolean processDragTo( UnPressedEvent e ) {
        ...
        if (elementInside == null){
        	...
        	//OLD INICIO Esto es codigo para quitar en futuro
            // Represents click anywhere in the screen. if some interaction has been initialized, trigger click fail effects
            int actionType = Game.getInstance().getActionManager( ).getActionSelected( );
            if (actionType == ActionManager.ACTION_DRAG_TO ){
            	Game.getInstance().getFunctionalPlayer( ).triggerClickFailEffects(draggingElement);
            }
            //OLD FIN
